...
function ComplexityPanel({ scene }: { scene: (typeof ALGORITHM_SCENES)[number] }) {
  return (
    <section className="grid grid-cols-2 gap-px overflow-hidden rounded-[6px] border border-white/10 bg-white/10 sm:grid-cols-4">
      ...
      ].map(([label, value]) => <div key={label} className="bg-[#0c1710] p-3"><p className="font-mono text-xs font-bold text-white">{value}</p><p className="mt-1 text-[9px] font-bold uppercase text-white/35">{label}</p></div>)}
    </section>
  )
}
...
function PredictionGate({ quiz, onAnswer }: { quiz: SceneQuiz; onAnswer: (correct: boolean) => void }) {
  ...
  return (
    <section className="rounded-[7px] border border-[var(--amber)] bg-[var(--amber-soft)] p-4">
      ...
      <div className="mt-3 grid gap-2">{quiz.options.map((option, index) => <button key={option} type="button" disabled={revealed} onClick={() => setSelected(index)} className={cn('rounded-[5px] border px-3 py-2 text-left text-xs font-semibold', revealed && index === quiz.answer ? 'border-[var(--accent)] bg-[var(--accent-soft)] text-[var(--accent-strong)]' : revealed && selected === index ? 'border-[var(--red)] bg-[var(--red-soft)] text-[var(--red)]' : selected === index ? 'border-[var(--amber)] bg-[var(--surface)]' : 'border-[var(--border)] bg-[var(--surface)]')}>{option}</button>)}</div>
...
export function AlgorithmLabPage() {
  ...
  return (
    <div className="min-h-screen bg-[#07100b] text-white">
      <header className="sticky top-0 z-40 flex h-16 items-center gap-3 border-b border-white/10 bg-[#07100b]/95 px-3 backdrop-blur-md sm:px-5">
        <IconButton icon={ArrowLeft} label="Return to Mentor" onClick={() => navigate('/mentor')} className="border-white/10 bg-white/5 text-white/70 hover:border-[#4ec08c] hover:text-[#4ec08c]" />
        <div className="min-w-0 flex-1"><div className="flex items-center gap-2"><Box size={16} className="text-[#4ec08c]" /><p className="truncate text-sm font-extrabold">3D Algorithm Lab</p></div><p className="mt-0.5 truncate text-[9px] font-bold uppercase text-white/35">{completedCount}/{ALGORITHM_SCENES.length} labs completed</p></div>
      ...
      <div className="grid min-h-[calc(100vh-4rem)] lg:grid-cols-[280px_minmax(0,1fr)]">
        <aside className={cn('fixed inset-y-0 left-0 z-50 flex w-[min(88vw,320px)] flex-col border-r border-white/10 bg-[#0a1510] transition-transform lg:sticky lg:top-16 lg:z-20 lg:h-[calc(100vh-4rem)] lg:w-auto lg:translate-x-0', catalogueOpen ? 'translate-x-0' : '-translate-x-full')}>
        ...
        <main className="min-w-0">
          <section className="grid border-b border-white/10 xl:grid-cols-[minmax(0,1.45fr)_minmax(320px,.55fr)]">
            <div className="relative h-[52vh] min-h-[380px] overflow-hidden xl:h-[calc(100vh-13rem)] xl:min-h-[560px]">
              ...
              <div className="pointer-events-none absolute inset-x-0 bottom-0 bg-gradient-to-t from-[#07100b] via-[#07100b]/80 to-transparent px-4 pb-4 pt-20 sm:px-6"><p className="text-[10px] font-extrabold uppercase text-[#74d6a8]">{frame.title}</p><p className="mt-1 max-w-3xl text-sm leading-6 text-white/75">{frame.narration}</p></div>
            </div>

            <aside className="border-t border-white/10 bg-[#0a1510] p-4 xl:max-h-[calc(100vh-13rem)] xl:overflow-y-auto xl:border-l xl:border-t-0 sm:p-5">
              <div className="flex items-center gap-2"><BookOpenCheck size={16} className="text-[#4ec08c]" /><p className="text-[10px] font-extrabold uppercase text-white/35">Mental model</p></div><p className="mt-2 text-sm leading-6 text-white/72">{scene.mentalModel}</p>
              <div className="mt-5"><p className="text-[10px] font-extrabold uppercase text-white/35">Invariant right now</p><p className="mt-2 rounded-[6px] border border-[#4ec08c]/25 bg-[#17392b] p-3 text-xs leading-5 text-[#a4e7c5]">{frame.invariant}</p></div>
          ...
          <footer className="sticky bottom-0 z-30 border-t border-white/10 bg-[#07100b]/95 px-3 py-3 backdrop-blur sm:px-5">
