...
export function FaangYearPage() {
  ...
  return (
    <div className="page-content">
      ...
      <section className="panel mb-4 overflow-hidden bg-[#17231c] text-white">
        <div className="grid gap-px bg-white/10 lg:grid-cols-[1.3fr_.7fr]">
          <div className="bg-[#17231c] p-6"><Badge tone="green">52-week operating plan</Badge><h2 className="mt-4 max-w-2xl text-2xl font-bold text-white">FAANG-range readiness is a portfolio of evidence, not a solved-count target.</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-white/65">Plan for 12 to 15 focused hours per week. DSA remains central, but CS fundamentals, one deep project, communication, referrals, and real interviews run in parallel.</p>{!startedAt && <Button className="mt-5" onClick={startYearPlan}><CalendarRange size={16} /> Start my 52 weeks</Button>}</div>
          <div className="bg-[#17231c] p-6"><p className="text-[10px] font-bold uppercase text-white/45">Current checkpoint</p><p className="metric-number mt-2 text-4xl font-extrabold text-white">Week {currentWeek}</p><p className="mt-1 text-sm font-bold text-white/80">{currentMonth.title}</p><div className="mt-5 h-1.5 overflow-hidden rounded-full bg-white/15"><div className="h-full bg-[var(--accent)]" style={{ width: `${progress}%` }} /></div><p className="metric-number mt-2 text-xs text-white/55">{completed.size}/52 weekly reviews complete</p>{startedAt && <p className="mt-3 text-[10px] text-white/40">Started {format(startedAt, 'MMM d, yyyy')} · target {format(addWeeks(startedAt, 52), 'MMM d, yyyy')}</p>}</div>
        </div>
      </section>
